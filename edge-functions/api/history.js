...
function recordKey(token,item){
  ...
  return prefix(token)+order+':'+item.id;
}
function keyName(entry){return typeof entry==='string'?entry:entry?.name}
...
export async function onRequestGet(context){
  try{
    ...
    const kv=getKV(context.env),url=new URL(context.request.url);
    ...
    return json({items,cursor:listed.cursor||null,complete:listed.list_complete??!listed.cursor});
...
export async function onRequestPost(context){
  try{
    if(!isSiteRequest(context.request))return json({error:'不允许跨站访问。'},403);
    const token=historyToken(context.request);if(!token)return json({error:'请先在设置中保存 API Key。'},401);
    const item=cleanItem((await context.request.json()).item),kv=getKV(context.env),historyKey=recordKey(token,item);
...
export async function onRequestDelete(context){
  try{
    ...
    await getKV(context.env).delete(historyKey);
    return json({ok:true});
