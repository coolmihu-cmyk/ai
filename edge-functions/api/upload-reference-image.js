...
function makeObjectKey(extension,token){const date=new Date(),year=date.getUTCFullYear(),month=String(date.getUTCMonth()+1).padStart(2,'0'),day=String(date.getUTCDate()).padStart(2,'0');const id=crypto.randomUUID?crypto.randomUUID():Date.now().toString(36)+'-'+Math.random().toString(36).slice(2);return 'references/'+year+'/'+month+'/'+day+'/key-'+token+'/'+id+'.'+extension}
...
export async function onRequestPost(context){
  try{
    ...
    const config=readConfig(context.env),objectKey=makeObjectKey(extension,token),host=config.COS_BUCKET+'.cos.'+config.COS_REGION+'.myqcloud.com',pathname='/'+objectKey,now=Math.floor(Date.now()/1000),keyTime=now+';'+(now+900);
    ...
    const upload=await fetch('https://'+host+pathname,{method:'PUT',headers:{Authorization:authorization,'Cache-Control':headers['cache-control'],'Content-Type':contentType,'x-cos-forbid-overwrite':'true'},body:await file.arrayBuffer()});
    if(!upload.ok)throw new Error('COS 写入失败（HTTP '+upload.status+'）。');
    return json({url:config.COS_PUBLIC_BASE_URL.replace(/\/+$/,'')+'/'+objectKey,key:objectKey,contentType});
