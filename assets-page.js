...
function localEditSelectVersion(versionId,imageUrl,versionLabel=''){
  ...
  localEditSetError();loadLocalEditImage(version,{focus:true}).catch(()=>{});
}
...
const LOCAL_EDIT_QUALITY_WARNING_SEEN_KEY='mihu-local-edit-quality-warning-seen-v1';
function localEditShouldShowQualityWarning(){
  try{if(localStorage.getItem(LOCAL_EDIT_QUALITY_WARNING_SEEN_KEY)==='1')return false;localStorage.setItem(LOCAL_EDIT_QUALITY_WARNING_SEEN_KEY,'1');return true}
  catch(_){return true}
}
...
function localEditUpdateImageFrame(){localEditSetImageFrame(localEdit.image)}
...
function localEditShowOriginalPreview(){if(localEdit.submitting||localEdit.guiding)return;const original=localEdit.versions[0];if(original)loadLocalEditImage(original,{focus:true}).catch(()=>{})}
...
function openLocalEdit(item,trigger,{versions=[item],resume=false,threadPosition='current'}={}){
  ...
  const showQualityWarning=!resume&&localEditShouldShowQualityWarning();
  localEdit.lastFocus=trigger||document.activeElement;localEdit.versions=orderedVersions;localEdit.editRootId=String(orderedVersions[0]?.id||item.editRootId||item.id);localEdit.editGroupId=item.editGroupId||'edit-'+localEdit.editRootId;localEdit.messages=resume?localEditMessagesForVersions(orderedVersions):[...(showQualityWarning?[{role:'assistant',text:LOCAL_EDIT_QUALITY_WARNING,qualityWarning:true}]:[]),{role:'assistant',text:LOCAL_EDIT_WELCOME},...localEditMessagesForVersions(orderedVersions)];localEditSetComposerCollapsed(false);localEditClearReference();
...
localEdit.originalPreview.onclick=localEditShowOriginalPreview;
...
localEdit.stage.addEventListener('pointerdown',event=>{if(event.button!==0||event.target.closest('button')||!localEdit.image.src)return;event.preventDefault();const view=localEdit.view;view.pointerId=event.pointerId;view.startX=event.clientX;view.startY=event.clientY;view.originX=view.x;view.originY=view.y;localEdit.stage.setPointerCapture(event.pointerId);localEdit.stage.classList.add('is-panning')});
...
async function syncPublishedAssetIds(){
  try{
    const response=await fetch('/api/public-references',{headers:{Accept:'application/json'},cache:'no-store'}),data=await response.json().catch(()=>({}));
    if(!response.ok)throw new Error(data.error||'公共参考库状态读取失败。');
    const publishedUrls=new Set((Array.isArray(data.items)?data.items:[]).map(item=>String(item?.imageUrl||'')).filter(Boolean));
    const confirmed=assetItems.filter(item=>publishedUrls.has(String(item.url||''))).map(item=>String(item.id));
    localStorage.setItem(PUBLISHED_ASSET_IDS_KEY,JSON.stringify(confirmed));
    return new Set(confirmed);
  }catch(error){console.warn('公共参考发布状态同步失败',error);return publishedAssetIds()}
}
...
async function openAssetPublish(item,button){
  await syncPublishedAssetIds();
  const published=publishedAssetIds().has(String(item.id));setAssetPublishButton(button,published);
  if(published)return;
...
requestAnimationFrame(async()=>{
  ...
  syncPublishedAssetIds().then(()=>renderAssets());
  syncCloudHistory();
